defmodule PlausibleWeb.Live.Sites do
  ...
  defp load_sites(%{assigns: assigns} = socket) do
    ...
    invitations = extract_invitations(sites.entries, assigns.current_user, assigns.my_team)
  ...
  defp extract_invitations(sites, user, team) do
    sites
    |> Enum.filter(&(&1.entry_type == "invitation"))
    |> Enum.flat_map(& &1.invitations)
    |> Enum.map(&check_limits(&1, user, team))
  end

  defp check_limits(%{role: :owner, site: site} = invitation, user, team) do
